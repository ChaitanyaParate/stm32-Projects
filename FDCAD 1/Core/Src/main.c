  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  if (HAL_FDCAN_ConfigFilter(&hfdcan1, &sFilterConfig) != HAL_OK)
	    ...
	    // Prepare the transmission header
	    TxHeader.Identifier = 0x321;  // Standard ID
	    ...
	    // Wait for message to be received (polling mode)
	    while (HAL_FDCAN_GetRxMessage(&hfdcan1, FDCAN_RX_FIFO0, &RxHeader, RxData) != HAL_OK)
	    {
	      // Polling until a message is received
	    }

	    // Data has been received, you can now compare or print the data
