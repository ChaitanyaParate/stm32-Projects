...
void Task3_init(void const * argument)
{
  /* USER CODE BEGIN Task3_init */
	for(;;)
	  {
	    // Wait for the semaphore (blocks until it is available)
	    if (osSemaphoreWait(myBinarySem01Handle, osWaitForever) == osOK) {
	      // Critical section: Add the code that should execute when the semaphore is acquired.

	      // Example: Toggle GPIO pin (PA0)
	      HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_0);

	      // No need to release the semaphore here. Task3 simply waits for Task4 to release it.
	    }
...
void StartTask03(void const * argument)
{
  ...
  /* Infinite loop */
	for(;;)
	  {
	    // Task4 will signal Task3 by releasing the semaphore
	    // Example: Toggle GPIO pin (PA1)
